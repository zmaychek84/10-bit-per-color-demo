...
HRESULT __stdcall GrayscaleEffect::CreateGrayscaleEffectImpl(_Outptr_ IUnknown** ppEffectImpl)
{
    // Since the object's refcount is initialized to 1, we don't need to AddRef here.
    *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) GrayscaleEffect());
...
HRESULT GrayscaleEffect::Register(_In_ ID2D1Factory1* pFactory)
{
    ...
    PCWSTR pszXml =
        XML(
            ...
            <Effect>
                ...
                <Property name='Description' type='string' value='Renders gray scale at varying bit depths'/>
                <Inputs />
    ...
    return pFactory->RegisterEffectFromString(
        CLSID_CustomGrayscaleEffect,
        pszXml,
        bindings,
        ARRAYSIZE(bindings),
        CreateGrayscaleEffectImpl
        );
}

IFACEMETHODIMP GrayscaleEffect::Initialize(
    ...
    )
{
    ...
    try
    {
        DX::ReadDataFromFile(DX::GetAbsolutePath(std::wstring(L"GrayscaleEffect.cso")).c_str(), &m_shaderdata, &size);
    }
    catch (std::exception)
    {
    ...
    if (SUCCEEDED(hr))
    {
        hr = pEffectContext->LoadPixelShader(GUID_GrayscaleEffectPixelShader, m_shaderdata, size);
    }
...
HRESULT GrayscaleEffect::UpdateConstants()
{
...
IFACEMETHODIMP GrayscaleEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP GrayscaleEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP GrayscaleEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
    m_drawInfo = pDrawInfo;

    return m_drawInfo->SetPixelShader(GUID_GrayscaleEffectPixelShader);
}

// Calculates the mapping between the output and input rects.
IFACEMETHODIMP GrayscaleEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP GrayscaleEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP GrayscaleEffect::MapInvalidRect(
    UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) GrayscaleEffect::GetInputCount() const
{
    return 0;
}
...
IFACEMETHODIMP_(ULONG) GrayscaleEffect::AddRef()
{
    m_refCount++;
    return m_refCount;
}

IFACEMETHODIMP_(ULONG) GrayscaleEffect::Release()
{
    m_refCount--;
...
IFACEMETHODIMP GrayscaleEffect::QueryInterface(
    _In_ REFIID riid,
...
// Size in pixels.
HRESULT GrayscaleEffect::SetOutputSize(D2D1_POINT_2F size)
{
    m_constants.outputSize = size;
    return S_OK;
}

D2D1_POINT_2F GrayscaleEffect::GetOutputSize() const
{
